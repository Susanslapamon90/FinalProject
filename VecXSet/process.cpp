...
void clear_exist_ID(set<string> &listset2,const Aset &aset){
	set<string>::iterator ti;
	int count = 0;
	for(ti = listset2.begin(); ti != listset2.end(); ti++){
		if(aset[int((*ti)[0])].find(INDEX<THistory>(*ti)) != aset[int((*ti)[0])].end()){
			//cout <<"clear "<< *ti << endl;
			listset2.erase(ti);
		}
		else
...
void listing10(bool exist, string &ID,const Aset &aset){
	int i;
	Set::iterator si;
	set<TL> listset;
	set<TL>::iterator ti;
	if(exist){
		for(int j = 0; j < 128; j++){
			for(si = aset[j].begin(); si != aset[j].end(); si++){
...
void processLogin(Aset& aset,customer** user_now){
	...
	Set::iterator tmp = aset[num].find(id_tmp);
	if(tmp == aset[num].end())
...
void processCreate(Aset &aset){
	...
	int num = int(ID[0]);
	INDEX<THistory> id_tmp(ID,cus_temp);
	if(aset[num].find(id_tmp) == aset[num].end()){
		INDEX<THistory> *new_index = new INDEX<THistory>(ID,PW);
		aset[num].insert(*new_index);
		cout <<"success"<< endl;
		delete new_index;
	}else{
		cout <<"ID "<< ID <<" exists, ";
		listing10(false, ID, aset);
	}
}
...
void processDelete(Aset &aset){
	...
	int num = int(ID[0]);
	Set::iterator tmp = aset[num].find(id_temp);
	if(tmp == aset[num].end())
		cout <<"ID "<< ID <<" not found"<< endl;
	else if(!tmp-> cu_ptr -> authenticated(PW))
		cout <<"wrong password"<< endl;
...
void processMerge(Aset &aset){
	...
	Set::iterator tmp1 = aset[num1].find(id_tmp1);
	Set::iterator tmp2 = aset[num2].find(id_tmp2);
	if(tmp1 == aset[num1].end())
		cout <<"ID "<< ID1 <<" not found"<< endl;
	else if(tmp2 == aset[num2].end())
...
void processTransfer(customer* user_now, int& TIME_CNT, Aset &aset){
	...
	INDEX<THistory> id_tmp(ID,cus_temp);
	int n = int(ID[0]);
	Set::iterator tmp = aset[n].find(id_tmp);
	if(tmp == aset[n].end()){
		cout <<"ID "<< ID <<" not found, ";
		listing10(true, ID, aset);
