...
void clear_exist_ID(set<string> &listset2,const Map &idmap){
	set<string>::iterator ti;
	int count = 0;
	for(ti = listset2.begin(); ti != listset2.end(); ti++){
		if(idmap.find(*ti) != idmap.end()){
			//cout <<"clear "<< *ti << endl;
			listset2.erase(ti);
		}
		else
...
void listing10(bool exist, string &ID, Map &idmap){
	int i;
	Map::iterator mi;
	set<TL> listset;
	set<TL>::iterator ti;
...
void FIND(string user, string test, Map& idmap){
	int mode = -1;
//	bool found = false;
...
void processLogin(Map& idmap,customer** user_now){
	...
	string ID, PW;
	cin >> ID >> PW;
	Map::iterator tmp = idmap.find(ID);
	if(tmp == idmap.end())
...
void processMerge(Map &idmap){
	...
	cin >> ID1 >> PW1 >> ID2 >> PW2;
	Map::iterator tmp1 = idmap.find(ID1);
	Map::iterator tmp2 = idmap.find(ID2);
	if(tmp1 == idmap.end())
		cout <<"ID "<< ID1 <<" not found"<< endl;
	else if(tmp2 == idmap.end())
...
void processFind(customer* user_now, Map &idmap){
	string wild_card_ID;
	cin >> wild_card_ID;
	FIND(user_now->ID, wild_card_ID, idmap);
	cout << endl;
}
